...
hotels_recipe <- recipe(is.canceled ~ ., 
                       data = house_training) %>% 
  step_mutate(children_ind = ifelse(children == 0, 0, 1),
              babies_ind = ifelse(babies == 0, 0, 1),
              previous_cancellations_ind = ifelse(previous_cancellations == 0, 0, 1),
              agent_ind = ifelse(agent = NULL, 0, 1),
              company_ind = ifelse(company = NULL, 0, 1)) %>%
